...
import { Search, FileText, MapPin, Hash, ChevronDown } from "lucide-react";
...
const STATUS_COLORS: Record<string, { bg: string; color: string; border: string }> = {
  ...
  in_progress: { bg: "#e8f0fe", color: "#1a56b0", border: "#a0c0f0" },
  resolved:    { bg: "#e6f4ea", color: "#1a7340", border: "#82c99a" },
...
export default function ClaimsPage() {
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: (updated) => {
      queryClient.setQueryData<Complaint[]>(["external-complaints"], (old = []) =>
        old.map((c) => (c.id === updated.id ? updated : c))
      );
      setSelected(updated);
      toast({ title: "Statut mis à jour", description: `Réclamation #${updated.tracking_number} mise à jour.` });
    },
    onError: () => {
        ...
        >
          ...
          <div className="px-6 py-5 shrink-0" style={{ borderBottom: "1.5px solid #dde0f0", background: "rgba(255,255,255,0.55)" }}>
            ...
            <p className="text-sm" style={{ color: "#7b72b0" }}>
              Réclamations soumises par les citoyens — {complaints.length} au total
            </p>
          </div>
          ...
          <div className="px-5 py-3 shrink-0" style={{ background: "rgba(255,255,255,0.35)" }}>
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Rechercher par nom, wilaya, numéro…"
                value={search}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className="flex items-center gap-1" style={{ color: isActive ? "rgba(255,255,255,0.6)" : "#9090b0" }}>
                        <Hash className="w-3 h-3" /> {c.tracking_number}
                      </span>
            ...
            <div className="max-w-xl mx-auto space-y-5">
              ...
              <div className="rounded-2xl p-5" style={{ background: "linear-gradient(135deg, #5b4d90, #7b65b0)", color: "white" }}>
                ...
                <div className="flex items-center gap-2 text-xs opacity-70">
                  <Hash className="w-3 h-3" /> {selected.tracking_number}
